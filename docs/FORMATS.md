...
>
> **Source de vérité** : la logique de format vit dans les modules ES `domain/corpus.mjs`
> (`parseCorpus`/`serializeCorpus`) et `domain/sonal.mjs` (`parseSonal`), consommés par le
...
### Invariants verrouillés par le golden (`domain/sonal.mjs`, testé via jsdom)
